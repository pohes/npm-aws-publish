...
class ElasticBeanstalkDeployment extends require('../AWSDeployment') {
   ...
   */
  async createApplicationIfNeeded() {
    try {
      console.log(`trying to create a new application ${this.deploymentConfig.ApplicationName}`)
      let promiseResult = await this.eb.createApplication({ApplicationName: this.deploymentConfig.ApplicationName}).promise()
      console.log(`created a new application : `)
      console.log(promiseResult.Application)
    } catch (e) {
